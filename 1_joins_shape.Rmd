...
cod_collapse %>% filter(collapse==TRUE) %>% summarize(tsyear=min(tsyear)) %>% .$tsyear

# Plot the catch time series and the collapse year
ggplot() + 
  geom_line(aes(y=total_catch, x=tsyear, color=collapse), data=cod_collapse) +
  geom_vline(xintercept = cod_collapse %>% filter(collapse==TRUE) %>% summarize(tsyear=min(tsyear)) %>% .$tsyear) + # Draws vertical line
